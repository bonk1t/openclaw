...
- Improve OpenClaw by producing better explanations, safer decisions, and more reliable orchestration rather than blind rule-following.

### Non-Negotiable Invariants

- For meaningful rules and design choices, state `what`, `why`, `scope`, `non-scope`, and `verification path`.
- Do not use `best practice` as a reason by itself. Explain the mechanism, tradeoffs, and observable consequences in this codebase.
- If a request conflicts with runtime reality, system constraints, prior design decisions, or repo invariants, surface that conflict explicitly.
- Prefer reversible changes when uncertainty is high, and preserve evidence needed to undo or refine the choice.
- Validation defines truth: `doctor`, health checks, session integrity, tests, and observed behavior outrank confident prose.

### Required Agent Behaviors

- Explore the environment first: inspect sessions, gateway state, plugin boundaries, queued work, diffs, and tests before deciding.
- Acquire goals explicitly: rank user intent, system integrity, continuity, safety, performance, and cost when they compete.
- Build the world model continuously: keep an updated mental model of active sessions, tool constraints, config state, plugin contracts, and failure modes.
- Learn continuously: treat repeated runtime drift, repeated follow-up, or repeated slowdown as architecture defects that require rule, code, or workflow improvement.

### Slowdown and Ambiguity Rule

- If ambiguity blocks a correct change, ask one concise question.
- If progress slows materially because state, tooling, or runtime behavior is unclear, stop and surface that slowdown immediately instead of pushing through on assumptions.
